...
SYSLOG_APP_LOG_FORMAT = '1 %(asctime)s [app_name] %(funcName)s %(message)s'
QLOGGER = 0
...
def create_log():
    ...
    if not util_qpylib.is_sdk():
        ...
        app_name = app_qpylib.get_app_name()
        if util_qpylib.is_ipv6_address(console_ip):
            console_ip = console_ip[1:-1]
        syslog_handler = SysLogHandler(address=(console_ip, 514))
        syslog_handler.setFormatter(logging.Formatter(SYSLOG_APP_LOG_FORMAT.replace('[app_name]', app_name)))
